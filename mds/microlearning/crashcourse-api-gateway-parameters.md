...
There are two main parameters when working with API's:
- Path
	- Part of the path
	- Mandatory
- Query
...
The notation of a Path parameter is the name of the path parameter encapsuled with curly brackets. In this case the path parameter label is uuid.
When a external client would call this endpoint they will replace the parameter placeholder with a valid uuid for which they want to retrieve the order information.
A example would be:

- /orders/12345

If a order with that specific uuid exists in our backend we will return the information to the client. 
...
The second parameter that is used frequently is the query parameter. Query parameters are mostly optional in nature and can help you to filter or sort a collection of resources.
...
- Path parameters are always mandatory wheras query parameters are mostly optional
...
Once again if they have a solid base re-use that, eMagiz will include path and query parameters if they exists in the backend operation.
...
First step is to open the API Gateway Catalog. 
...
The second step would be to add this parameter and link it to this resource path. You can do that in the same screen. On the right side of the screen you have the parameters tab
...
The rest is correct by default. For illustrative purposes it would be good to add a example of a value that is expected. Save when ready
...
For query parameters we don't need to do so because they are always a extension of the resource path at the **end** of a resource path.
...
To add a query parameter you give the parameter a name here and you select the option Query instead of Path. A example of this filled in looks as follows.
