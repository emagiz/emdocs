...
In this microlearning we will educate you how you can split messages with the help of the eMagiz tooling.
...
## 2. Key concepts
This micro learning focuses on splitting messages.
...
With the help of the Xpath splitter you can split the message based on the result of a Xpath expression.

According to the helptext of eMagiz: "The splitter uses the provided XPath expression to split the payload into a number of nodes. 
By default this will result in each Node becoming the payload of a new message."

As you remember from our microlearnings on Xpath a XPath navigates through the input XML based on the expression given.
What the starting point of this navigation is differs based on the context. 
If you want to write a XPath while doing a transformation any input element can be the starting point of your XPath.
However when you write a XPath outside of the transformation tooling the root node of your input message will always be the starting point for your XPath.
...
To do so we need to place a XPath splitter before validating the system message. 
As a reminder, the best practice is that after every mutation on message level you do a validation to check your work.
...
Now it becomes time for the crucial part of this component. Determining the correct XPath to split my message in order to end up with a valid system message.
...
- Always start at the root of the input message when writing a XPath outside of the transformation
...
Furthermore we give you a example of how such a input message could look like:
...
With the help of a example message or a documentation dictating the structure of a message (i.e. a XSD) you can quickly see whether a message has a namespace.
...
The begin of the XPath should look like this: /Projects. But that is not all. 
