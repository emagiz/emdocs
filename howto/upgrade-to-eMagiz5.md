...
Here are some bus health checks that you should do after the migration is finished and every flow seems to be running in the runtime dashboard:   
 -  **Test any flow where the user made any change related to the HornetQ JMS settings OR customised the autogenerated flows(jms server, container infra, connector infra) OR the autogenerated part of the flows(onramp, offramp, entry connector, exit connector).**
 
 -  **To make sure the migration has been indeed succesfull, it is best to test the all the flows which contain unusual structures such as an entry/exit connector**

 #### Large message issues
  - when sending a message from HornetQ client, a message is considered to be **large** when exceds the default value of **100KB**
  - when sending a message from AMQP client, a message is considered to be **large** when exceds the default value of **500KB**
